...
graph TB
    ...
    subgraph Backend["Backend (Node.js / Hono)"]
        ...
        AgentWorker["Agent Worker / Agent<br>(Node.js)"]:::component
...
- **Agent Worker**: Manages usage of the AI Agent for tasks like "Patch this canvas" or answering questions. Interacts with the MCP server.
...
- **Redis (BullMQ)**: Handles the asynchronous task queue for the Workflow Worker and Agent, ensuring reliable execution and retries for generative tasks.
