...
import { Button, Label, Separator, Slider, cn } from "@gatewai/ui-kit";
import { Brush, Eraser, PaintBucket } from "lucide-react";
import { memo, useCallback, useEffect, useMemo, useRef, useState } from "react";
import { PaintDimensionsConfig } from "./paint-config/index.js";
import { colorsSimilar, colorToRgb, getPixel, setPixel } from "./utils.js";

const PaintNodeComponent = memo((props: { selected: boolean; id: string; dragging: boolean }) => {
    ...
    const performFloodFill = useCallback(
        (isPreview: boolean, posX: number, posY: number) => {
            ...
            if (isPreview) {
                ...
                alpha = Math.floor(brushA * 0.5); // Semi-transparent for preview
            } else {
                ctxToPut = maskCtx;
    ...
    const handleMouseMove = useCallback(
        (e: React.MouseEvent<HTMLCanvasElement>) => {
            ...
            const step = Math.max(1, Math.min(5, radius / 4));
