...
export class ImageCompositorProcessor implements NodeProcessor {
	...
	async process({
		...
		try {
			...
			const inputHandlesWithValues = this.graph.getAllInputValuesWithHandle(data, node.id);
			...
			for (const layerConfig of allLayers) {
				...
				if (layerConfig.type === "Image" && input.type === DataType.Image) {
					const imgBuffer = await this.media.getImageBuffer(input.data as FileData);
