...
const imageGenProcessor: BackendNodeProcessor = async ({
	node,
	data,
	prisma,
	services,
	...
	try {
		...
		for (const imgData of imageFileData || []) {
			...
			const arrayBuffer = await services.getFromGCS(key, bucket);
		...
		const dimensions = await services.getImageDimensions(buffer);
		...
		await services.uploadToGCS(buffer, key, contentType, bucket);
