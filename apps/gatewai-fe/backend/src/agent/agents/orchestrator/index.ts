...
- Looping connection is forbidden
- Cannot connect an Input handle to another Input handle or an Output handle to another Output handle
- To be able to connect an Input and Output Handle, they should share at least one DataType
- An Input handle can only have one connection, but Output handle may have many.
`;
	...
	const OrchestratorAgent = new LlmAgent({
		...
		beforeModelCallback: ({ request }) => {
			...
			if (request?.config) {
				...
				).replace("{{node_templates_list}}", NodeTemplatesList);
			}
			return undefined;
		},
	});
